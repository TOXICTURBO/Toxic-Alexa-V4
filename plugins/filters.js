...
if(pattern.test(message.message)){
    ...
    Asena.addCommand({on: 'text', fromMe: false}, (async (message, match) => {   
        ...
        if(banned !== undefined) return
        if (!!message.mention && message.mention[0] == '916380260672@s.whatsapp.net') {
await message.client.sendMessage(message.jid, fs.readFileSync('./Kaztroserv2/owner.mp3'), MessageType.audio, { mimetype: Mimetype.mp4Audio,duration: Config.SAID, contextInfo: { forwardingScore: 5, isForwarded: true }, quoted : message.data, ptt: true})
        }
        if (!!message.mention && message.mention[0] == Config.MENTION) {
...
Asena.addCommand({on: 'text', fromMe: false}, (async (message, match) => {
    ...
    if(banned !== undefined) return
    if (!!message.mention && message.mention[0] == '916380260672@s.whatsapp.net') {
await message.client.sendMessage(message.jid, fs.readFileSync('./Kaztroser/Turbo.webp'), MessageType.sticker, { mimetype: Mimetype.webp, quoted : message.data, ptt: false})
    }
const array = ['Sed','Hi']
array.map( async (a) => {
let pattern = new RegExp(`\\b${a}\\b`, 'g');
